...
Now that we have a basic understanding of how TypeScript works and how to create small projects with it, it's time to start building something actually useful. We are now going to create a new project with a bit more realistic use cases in mind.
...
```
 ...
 Now we can initialize our tsconfig.json settings by running:
...
Take the time to create a proper setting for yourself and your team, so in the long run, everything runs smoothly. 
...
Now we can finally start coding! As always, we begin by creating a ping-endpoint, just to make sure everything is working. 
...
When we run the app with <i>npm start</i>, we can verify that also the production build works.
...
Initialize project that will be used by the frontend. Configure eslint and tsconfig with the same configurations that are used in the material. Define an endpoint that responses to HTTP GET requests to route <i>/ping</i>.   
...
The name service originated from [Domain driven design](https://en.wikipedia.org/wiki/Domain-driven_design) and was made famous by the [Spring](https://spring.io/) framework.
...
But since the JSON already has its values declared, assigning a type for the data set results in error:
...
We can fix the problem by doing [type assertion](http://www.typescriptlang.org/docs/handbook/basic-types.html#type-assertions). This should only be done if we are sure we know what we are doing. 
...
Consider a page for listing some data, some of which is sensitive, and some of which is non-sensitive.
...
In our project, we should consider that Ilari might want to create a listing of all his diary entries <i>excluding</i> the comment field. During a terrifying flight, he might end up writing something he wouldn't necessarily want to show anyone else. 
...
The pick can be used to either construct an entirely new type or to inform a function of what it should return on runtime. 
Utility types are special kinds of type tools, but they can be used just like regular types.
...
This happens because [TypeScript only checks](http://www.typescriptlang.org/docs/handbook/type-compatibility.html), whether we have all of the required fields or not, but excess fields are not prohibited. In our case, this means that is <i>not prohibited</i> to return an object of type <i>DiaryEntry[]</i>. Still, if we were to try to access the <i>comment</i> field, it would not be possible because we would be accessing a field that TypeScript is unaware of even though it exists.
...
Unfortunately, this can lead to unwanted behavior if you are not aware of what you are doing; the situation is valid as long as TypeScript is concerned, but you are most likely allowing use that is not wanted. 
...
The issue is that there is no guarantee that entry with the specified id can be found. 
It is good that we are made aware of this potential problem already at the compile phase. Without TypeScript, we would not be warned about this problem, and in the worst-case scenario, we could have ended up returning an <i>undefined</i> object instead of informing the user about the specified entry not being found. 
...
Applications rarely work entirely on their own, and we are forced to live with the fact that data from sources outside of our system cannot be fully trusted.
...
In our case, this is not apparent in the editor at all. Still, if we start looking at the variables more closely and hover over any of them, we can see that each of them is indeed type [any](http://www.typescriptlang.org/docs/handbook/basic-types.html#any). The editor does not complain in any way when we pass this data to the <i>addDiary</i>  function as parameters:
